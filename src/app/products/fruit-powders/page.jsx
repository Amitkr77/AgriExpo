...
const FILTERS = {
  category: ["all", "tropical", "berry", "citrus", "tangy", "tropical-fat"],
  cert: ["all", "organic", "natural", "fssc22000"],
};
  ...
  return (
    <button
      ...
      className={`px-4 py-2 rounded-full text-sm font-bold transition-all capitalize ${
        active ? "bg-[#fec567] text-[#765100]" : "bg-[#e4e3d4] text-[#1b1c13] hover:bg-[#dbdbcc]"
      }`}
    >
      {label === "tropical-fat" ? "tropical fat" : label}
    </button>
  );
}
...
function FeaturedHero({ product }) {
  ...
  return (
    <motion.div
      ...
      className="rounded-[3rem] bg-[#fec567]/20 border border-[#fec567]/40 p-6 md:p-8 mb-8 overflow-hidden"
    >
      <div className="flex flex-col md:flex-row gap-8 items-center">
        <div className="flex-1">
          <div className="inline-flex items-center gap-2 mb-3">
            <Sparkles className="h-4 w-4 text-[#7e5700]" />
            <p className="text-xs font-bold uppercase tracking-widest text-[#7e5700]">Bestselling Fruit Powder</p>
          </div>
          ...
          <div className="mt-4 flex gap-3 flex-wrap text-sm">
            <span className="bg-[#fec567]/30 text-[#765100] px-3 py-1 rounded-full font-bold">{product.meshSize}</span>
            <span className="bg-[#fec567]/30 text-[#765100] px-3 py-1 rounded-full font-bold">Shelf Life: {product.shelfLife}</span>
            <span className="bg-[#fec567]/30 text-[#765100] px-3 py-1 rounded-full font-bold">MOQ: 25kg</span>
          </div>
          <Link
            href={`/products/fruit-powders/${product.slug}`}
        ...
        <div className="flex-1 w-full">
          <div className="relative h-[280px] rounded-[2rem] overflow-hidden bg-[#fec567]/20 flex items-center justify-center">
...
export default function FruitPowdersPage() {
  const [category, setCategory] = useState("all");
  const [cert, setCert] = useState("all");
  const [search, setSearch] = useState("");

  const filtered = useMemo(() => {
    return FRUIT_POWDERS.filter((p) => {
      return (
  ...
  const featured = FRUIT_POWDERS.find((p) => p.featured);
  ...
  return (
    <div className="min-h-screen bg-[#fbfaeb] px-4 md:px-10 py-10 mt-20">
      ...
      <nav className="text-xs text-[#414943] mb-6 flex items-center gap-2">
        ...
        <span>/</span>
        <span className="text-[#14422d] font-bold">Fruit Powders</span>
      </nav>

      {/* HEADER */}
      <div className="mb-10">
        <h1 className="text-5xl md:text-6xl font-extrabold text-[#1b1c13]">Fruit Powders</h1>
        <p className="text-[#414943] mt-3 text-lg max-w-2xl">
          10 spray-dried and freeze-dried fruit powders. Export-certified and available for bulk B2B supply. Serving beverage brands, nutraceuticals, and food manufacturers worldwide.
        </p>
        <div className="mt-4 flex gap-3 flex-wrap">
          <span className="bg-[#fec567] text-[#765100] text-xs px-3 py-1 rounded-full font-bold">10 SKUs Available</span>
          <span className="bg-[#eae9da] text-[#1b1c13] text-xs px-3 py-1 rounded-full font-bold">Spray-Dried & Freeze-Dried</span>
          <span className="bg-[#eae9da] text-[#1b1c13] text-xs px-3 py-1 rounded-full font-bold">MOQ: 25kg Bags</span>
          <span className="bg-[#eae9da] text-[#1b1c13] text-xs px-3 py-1 rounded-full font-bold">Samples on Request</span>
        </div>
      </div>
          ...
          <div className="sticky top-24 rounded-[3rem] bg-[#f0efe0] p-6 space-y-6">
            <div>
              ...
              <input
                ...
                placeholder="Search fruit powders..."
                className="w-full bg-white border border-[#c0c9c1] rounded-full px-4 py-2 text-sm outline-none focus:ring-2 focus:ring-[#fec567]"
              />
            </div>
            <div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {FILTERS.cert.map((c) => (
                  <Pill key={c} label={c === "fssc22000" ? "FSSC 22000" : c} active={cert === c} onClick={() => setCert(c)} />
                ))}
              </div>
            </div>
            <Link
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              ...
              {filtered.map((product) => (
                ...
                >
                  <div className="group relative rounded-[2.5rem] bg-[#f5f4e5] p-3 overflow-hidden h-full flex flex-col hover:-translate-y-2 transition-all duration-500 border border-black/[0.04]">
                    <Link href={`/products/fruit-powders/${product.slug}`} className="absolute inset-0 z-20" />
                    <div className="relative overflow-hidden rounded-[2rem] h-[260px] bg-[#fec567]/20 flex items-center justify-center">
                    ...
                    <div className="p-4 flex flex-col flex-1">
                      ...
                      <div className="mt-3 flex gap-2 flex-wrap">
                        <span className="text-[10px] bg-[#fec567]/30 px-2 py-1 rounded-full font-bold">{product.meshSize}</span>
                        <span className="text-[10px] bg-[#fec567]/30 px-2 py-1 rounded-full font-bold">{product.processingMethod}</span>
                      </div>
            ...
            {filtered.length === 0 && (
              <div className="col-span-3 text-center py-20 text-[#414943]">
                ...
                <button onClick={() => { setCategory("all"); setCert("all"); setSearch(""); }} className="text-[#fec567] font-bold underline">Clear filters</button>
              </div>
            )}
          </div>
        </div>
