...
export async function generateStaticParams() {
  return VEGETABLE_POWDERS.map((product) => ({
    slug: product.slug,
  }));
}
...
export async function generateMetadata({ params }) {
  ...
  const product = VEGETABLE_POWDERS.find(
    (item) => item.slug === slug
  );
...
export default async function VegetableProductPage({ params }) {
  const { slug } = await params;
    ...
    <div className="min-h-screen bg-[#fbfaeb] pt-32 pb-20">
      <div className="max-w-7xl mx-auto px-4 md:px-8">
        ...
        <nav className="text-xs text-[#414943] mb-8 flex items-center gap-2">
          ...
          <span>/</span>
          <a href="/products/vegetable-powders" className="hover:text-[#14422d]">Vegetable Powders</a>
          <span>/</span>
